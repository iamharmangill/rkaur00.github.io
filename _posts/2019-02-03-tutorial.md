...
Now we are going to add functionality by getting the data submitted in form. when user will press the submit button, we will send the data back to **server.js** file using post method. Since, we have used action for form to be a result page, add the following code to your previous code in servre.js file:
...
```
Now, at last I am just adding the background image using inline style at body tag, and, also using jquery validation for name and email fields, so that user cannot enter invalid data.To display the server side error, I am displaying it using the error variable coming from server.js file and similarly to display my result, I am using JavaScript expression to display it using data variable rendering from server.js file.Complete code for index.ejs file is:
