...
```
- The first line of code is used to set view engine which is .ejs file, Since it is under views folder, the second line of code provides the path for that.
...
- We are getting the text for checkboxes from words variable that we have rendered to this page using **server.js** code. Thus, we are generating dynamic content from **data.json** file.
Hence, the complete code for **server.js** is:
```
const express = require('express'); 
